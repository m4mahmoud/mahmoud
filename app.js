...
function showAlert(message, type = "auto") {
	...
	const detectedType = type === "auto"
		? (/نجاح|تم |بنجاح|success/i.test(text) ? "success"
			: /خطأ|فشل|غير صحيحة|محظور|مفقود|يرجى|يجب|error|invalid/i.test(text) ? "error"
			: "info")
		: type;
...
async function claimSingleUserSession(user) {
	...
	// setDoc مع merge يجعل تسجيل الدخول يعمل حتى لو كان حساب
	// Firebase موجودًا لكن مستند المستخدم في Firestore غير موجود.
	await setDoc(
...
function watchSingleUserSession(user) {
	...
	unsubscribeUserSession = onSnapshot(userRef, async (snap) => {
		...
		if (data.activeSessionId && data.activeSessionId !== activeSessionId) {
			...
			showAlert("تم تسجيل الدخول بهذا الحساب من جهاز أو جلسة أخرى. سيتم تسجيل خروجك من هذه الجلسة.", "warning");
			await signOut(auth);
		}
	});
}
...
function confirmAction(message, options = {}) {
	return new Promise((resolve) => {
		...
		wrapper.querySelector(".confirm-ok")?.addEventListener("click", () => finish(true));
		wrapper.querySelector(".confirm-cancel")?.addEventListener("click", () => finish(false));
		wrapper.querySelector(".confirm-dialog-overlay")?.addEventListener("click", () => finish(false));
	});
}
	...
	try {
		if (isSignUp) {
			...
			try {
				...
				// نجاح Firebase Authentication لا يتوقف على نجاح مزامنة الجلسة.
				// هذا يمنع ظهور رسالة فشل تسجيل الدخول إذا كانت Firestore Rules
				// تمنع تحديث activeSessionId.
				try {
					await claimSingleUserSession(userCredential.user);
				} catch (sessionError) {
					console.warn("تعذر مزامنة الجلسة الواحدة:", sessionError);
					showAlert("تم تسجيل الدخول بنجاح، لكن تعذر تفعيل حماية الجلسة الواحدة حاليًا.", "warning");
				}
			} finally {
		...
		if (error.code === "auth/email-already-in-use") {
			...
			showAlert("تمت محاولات تسجيل دخول كثيرة. انتظر قليلًا ثم حاول مرة أخرى.", "warning");
		} else if (error.code === "auth/network-request-failed") {
			showAlert("تعذر الاتصال بالخدمة. تأكد من اتصال الإنترنت وحاول مرة أخرى.", "error");
		} else if (error.code === "auth/user-disabled") {
			showAlert("هذا الحساب موقوف. تواصل مع إدارة المنصة.", "warning");
		} else {
		...
		try {
			...
			if (userDocSnap.exists()) {
				...
				if (currentUserData.isBanned) {
					showAlert("عذراً، هذا الحساب محظور حالياً. يرجى التواصل مع الإدارة.", "warning");
					await signOut(auth);
					return;
				}
				...
				if (!sessionSyncInProgress && !activeSessionId) {
					try {
						...
						// لا نطرد المستخدم من الحساب بسبب مشكلة مزامنة الجلسة.
						// تسجيل الدخول نفسه تم بنجاح عبر Firebase Authentication.
					}
				}
...
window.changeUserRole = async (uid, newRole) => {
	...
	if (await confirmAction(`هل أنت متأكد من تغيير دور هذا المستخدم إلى (${roleTranslations[newRole]})؟`, { title: "تغيير دور المستخدم", confirmText: "تغيير الدور" })) {
		try {
			await updateDoc(doc(db, "users", uid), { role: newRole });
			showAlert(`تم تغيير دور المستخدم إلى (${roleTranslations[newRole]}) بنجاح.`, "success");
		} catch (error) {
...
window.deleteUserRecord = async (uid) => {
	if (await confirmAction("هل أنت متأكد من حذف بيانات هذا المستخدم من قاعدة البيانات؟", { title: "حذف بيانات المستخدم", confirmText: "حذف نهائياً" })) {
...
window.toggleUserBlock = async (uid, isCurrentlyBanned) => {
	...
	if (await confirmAction(`هل أنت متأكد من ${action} هذا المستخدم؟`, { title: `${action} المستخدم`, confirmText: action })) {
...
window.sendResetPasswordLink = async (email) => {
	if (await confirmAction(`إرسال رابط إعادة تعيين كلمة المرور إلى ${email}؟`, { title: "إعادة تعيين كلمة المرور", confirmText: "إرسال الرابط" })) {
		try {
			await sendPasswordResetEmail(auth, email);
			showAlert("تم إرسال رابط إعادة التعيين بنجاح إلى بريد المستخدم.");
		} catch (err) {
			showAlert("تعذر إرسال رابط إعادة تعيين كلمة المرور. من فضلك حاول مرة أخرى.", "error");
		}
	}
};
...
window.deleteCourseRecord = async (courseId) => {
	if (await confirmAction("هل أنت متأكد من حذف هذا الكورس نهائياً؟", { title: "حذف الكورس", confirmText: "حذف نهائياً" })) {
		try {
...
addCourseForm?.addEventListener("submit", async (e) => {
	...
	try {
		...
		if (err.code === "permission-denied") {
			...
			showAlert("تعذر حفظ الكورس. تأكد من البيانات والصلاحيات ثم حاول مرة أخرى.", "error");
		}
	}
});
...
window.requestCourseEnrollment = async (courseId, courseTitle) => {
	...
	if (await confirmAction(`هل تريد إرسال طلب اشتراك في كورس (${courseTitle})؟`, { title: "تأكيد طلب الاشتراك", confirmText: "إرسال الطلب" })) {
		try {
			...
			showAlert("تم إرسال طلب الاشتراك بنجاح! في انتظار موافقة الإدارة أو المعلم.");
...
function extractYouTubeVideoId(inputUrl) {
	...
	return match?.[1] || (url.length === 11 && /^[A-Za-z0-9_-]+$/.test(url) ? url : "");
}
...
addLessonForm?.addEventListener("submit", async (e) => {
	...
	try {
		...
		showAlert("تعذر إضافة الحصة. تأكد من البيانات والرابط ثم حاول مرة أخرى.", "error");
	}
});

// حذف درس من الكورس
window.deleteLesson = async (courseId, lessonId) => {
	if (await confirmAction("هل أنت متأكد من حذف هذه الحصة؟", { title: "حذف الحصة", confirmText: "حذف الحصة" })) {
		try {
...
window.updateEnrollmentStatus = async (enrollmentId, newStatus) => {
	...
	if (await confirmAction(`هل أنت متأكد من ${actionText} طلب الاشتراك هذا؟`, { title: "تأكيد طلب الاشتراك", confirmText: actionText })) {
		try {
